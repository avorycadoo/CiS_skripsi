        ...
        <form action="{{ route('sales.updateConfiguration') }}" method="POST"> <!-- Updated route -->
            ...
            <div class="form-group">
                ...
                @foreach ($discounts as $discount)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="discounts[]" value="{{ $discount->id }}"
            ...
            {{-- <div class="form-group">
                ...
                @foreach ($discounts as $discount)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="discounts[]" value="{{ $discount->id }}"
                            id="discount{{ $discount->id }}" {{ $discount->statusActive ? 'checked' : '' }}>
                        <label class="form-check-label" for="discount{{ $discount->id }}">
                            {{ $discount->name }}
                        </label>
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach ($shippings as $shipping)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="shippings[]" value="{{ $shipping->id }}"
                            id="shipping{{ $shipping->id }}" {{ $shipping->statusActive ? 'checked' : '' }}
                ...
                @foreach ($payments as $payment)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="payments[]" value="{{ $payment->id }}"
                            id="payment{{ $payment->id }}" {{ $payment->statusActive ? 'checked' : '' }}
